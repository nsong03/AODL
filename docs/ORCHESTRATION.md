...
| 2026-09-02 | **M6 launched** (owner request): FFT-based independent checker for rendered RF samples. Owner decisions: default pupil model = full exp(iCV) + +1-order band selection ("bragg_band"; "weak" mode for cross-validation); placement = MotionPlan.check() + check_samples() + CI gate. Plan reviewed (du = Λ/8 alias arithmetic, carrier handling per mode, unfilled ⇒ transmission 1, motion-capped golden-ratio beat averaging, w²(Z) focus fits, normalization + retard_compensate traps). WO-21…WO-23 authored; Wave R (WO-21) dispatched. |
| 2026-09-02 | Wave R accepted (WO-21, `712f413`): 401 tests; weak-vs-literal pupil 4.4e-7, czt 4e-13, 2J₁(C)/C to 1.1e-8, splatter −40.8 dB re-detected from samples. Corrections ratified into WO-22: interpolation law is cubic (oversample=2 for tight gates), coverage bound uses grid.half_span/v, first alias is p = −7 (J₇; 2.3e-9 at C = 0.3). Wave S (WO-22) dispatched. |
| 2026-09-02 | Wave S accepted (WO-22, `9cd0495`, resumed across a usage-limit outage with zero loss): 438 tests, 7 notebooks; flagship check PASS in 20 s (0 off-lattice blobs), three corruptions FAIL legibly. Rulings ratified: uniform sub-times for commensurate beat combs (golden-ratio Fibonacci blind spots measured at 22 % schedule noise); co-moving pupil measurement (smear ∝ velocity error) — flagged for adversarial WO-23 scrutiny; fading-axis edge-line gate exclusions; flagship waist/uniformity tolerances opened per-drive with both-sided pins (IM3 ∝ C², ρ = 0.30 apodization). Wave T (WO-23) dispatched. |
| 2026-09-02 | Wave T verdict: **M6 ACCEPTED** (WO-23, fix `e7792c3`: per-channel band decimation + 3 docstrings). Checker verified genuinely independent: constants to 1e-8–1e-10; 10 %-slow drive caught at 37.8× gate; co-moving de-smear changes a corrupted residual by 1 % (removes smear, not error); timing-skew blind spot quantified (≈1.22 µs threshold); uniform-comb fallback + beat_std flag confirmed. Reconcile-before-tag: F-1 PLAN text (fixed by architect), F-2 opened uniformity passes a 36 % rung fault, F-3 remnants, F-6 coverage collapse on small fading arrays, F-8/9/10/11 nits → **WO-24 (Wave U) dispatched**. Backlog: F-5 flagship check 21 s (render-bound; per-frame windowed rendering would cut ~60 %), F-12 Eq. S29 skew modeling. |
| 2026-09-02 | Wave U (WO-24) closed on its third dispatch after two silent deaths from container restarts (first: zero progress; second: 303 lines into report.py, stashed then dropped once superseded) — commit `07461a3`, 449 tests (+1 xfail), 7 notebooks. **F-2 finding (load-bearing, not just fixed):** a per-trap time-median uniformity statistic was proposed to separate persistent hardware faults from transient hand-over ripple, on the theory that a real fault holds still while ripple varies by frame. Measured on the flagship it does NOT work — separation ratio exactly 1.00 (clean 0.1373 vs. a 36%-faulted rung 0.1373) — because the Shepard ladder *slides*: a persistently-broken rung only touches any single trap for 1 of 7 checked frames, so it presents as transient too. Ruled report-only (`Tolerances.uniformity_median: float\|None = None`) with a strict xfail documenting the gap; the same statistic IS gated with an 8x separation on non-sliding (plain Eq. S19) arrays, where the premise holds. Also landed: `gated_fraction` + `require_coverage` (closes the 2x2-fading silent-PASS hole), a bounded fading-blob whitelist (`blob_fading=1.2`, both-sided synthetic + real-drive pins), a genuine off-by-one bug fix in the beat-comb threshold, and F-3/7/8/9/10/11 docs/wording corrections. Pushed to `main` alongside the feature branch. |

## Known limitation carried forward (not a defect — a measured boundary of the method)

The FFT checker's intensity-uniformity gate cannot, by construction, distinguish "this
specific rung is broken" from "real hand-over ripple" *on a sliding Shepard ladder*,
because both present with the same per-trap duration. Closing this for real needs a
different verdict primitive — compare each frame's per-trap intensity against the
*predicted* fade-schedule pattern (Table II's cos^p/sin^p envelope at that trap's known
ladder position) rather than against the frame's own median — which is a new algorithm,
not a tolerance tweak. Filed as an M7-candidate idea, not scheduled.
